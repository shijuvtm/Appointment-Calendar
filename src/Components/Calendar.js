...
{isMobile ? (
      ...
      <div className="mobile-day-view">
        ...
        {getAppointmentsForDate(mobileSelectedDate).map(apt => (
          <div key={apt.id} className="appointment-item">
            ...
            <button className="delete-btn" onClick={() => deleteAppointment(apt.id)}>Delete</button>
          </div>
        ))}
      </div>
    </>
    ...
    <div className="calendar-container">
      <div className="calendar-header">
        <button onClick={() => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1))} className="nav-button">‹</button>
            ...
            >
              ...
              {dayAppointments.map(apt => (
                <div key={apt.id} className="appointment-indicator">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); deleteAppointment(apt.id); }} className="delete-btn">×</button>
                </div>
              ))}
            </div>
          );
    ...
    <div className="modal-overlay" onClick={() => setShowModal(false)}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="form-group">
          ...
          <input type="text" value={formData.patientName} onChange={(e) => setFormData({...formData, patientName: e.target.value})} placeholder="Enter patient name" />
        </div>
        <div className="form-group">
          <label>Doctor:</label>
        ...
        <div className="modal-buttons">
          ...
          <button onClick={addAppointment} className="save-btn">Save Appointment</button>
        </div>
      </div>
    </div>
  )}
